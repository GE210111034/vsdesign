...
class GitHubAuthenticationProvider {
    constructor(context, uriHandler, ghesUri) {
        ...
        this._supportsMultipleAccounts = false;
            ...
            */
            ...
            if (forcedLogin) {
                if (session.account.label !== forcedLogin) {
                    ...
                    const result = await vscode.window.showWarningMessage(vscode.l10n.t('Incorrect account detected'), { modal: true, detail: vscode.l10n.t('The chosen account, {0}, does not match the requested account, {1}.', session.account.label, forcedLogin) }, keepNewAccount, tryAgain);
                    if (result === tryAgain) {
            ...
            const sessionIndex = sessions.findIndex(this._supportsMultipleAccounts
                ? s => s.account.id === session.account.id && (0, utils_1.arrayEquals)([...s.scopes].sort(), sortedScopes)
                : s => s.id === session.id || (0, utils_1.arrayEquals)([...s.scopes].sort(), sortedScopes));
